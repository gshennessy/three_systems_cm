...
components remain well within the detector field of view.   On the other hand, this paper’s data consist of right
ascensions and declinations of both components of each system, with the absolute calibration to the ICRS provided by the
World Coordinate System\citep{2002A&A...395.1061G,2002A&A...395.1077C} headers.  
...
We processed each system independently. When the primary was unsaturated, a two-dimensional Gaussian was fit using the
Astropy implementation of the Levenberg–Marquardt algorithm with a least-squares statistic. When saturated, two points
were selected along each of the four diffraction spikes, and their intersections were computed using Cramer's rule
\citep{Cramer1750}. This method of determining the positions of saturated stars was also done by
Bond\citep{2017ApJ...840...70B,2015ApJ...813..106B,2020ApJ...904..112B}. 
Pixel coordinates were converted to right ascension and declination using Astropy WCS routines,
taking into account the difference between 1-based WCS indexing and the 0-based indexing used in Python. 

The parallax displacement was removed from both components prior to fitting proper motions. The proper motion in right
ascension was fit using $\mu_{\alpha*} = \dot{\alpha}\cos\delta$, following the standard astrometric convention
\citep{vanLeeuwen2007,Lindegren2021}. The declination used in the fit corresponds to the CM declination at epoch
J1991.25. 

Once the ICRF positions were calculated as a function of time, the center of mass of the three systems can be
calculated at each observation date, along with the astrometric parameters for the center of mass of the system. 
The initial starting point for each calculation was the Hipparcos position of the primary star of each system. 
The separation and position angle was calculated at the Hipparcos epoch of 1991.25 using the orbital parameters from 
Table~\ref{tab:orbits}, and the masses of the two stars in order to get the ICRF position of the center of mass of the system. For
each HST position in Table~\ref{tab:positions}, the effect of the finite parallax was removed from both the primary (observed) and
secondary's (calculated) position, the center of mass position calculated, and then the proper motions were fit. The proper
motion in right ascension
is reported using $RA*\cos(Dec)$ as the current standard in astrometry\citep{1997A&A...323L..49P}. The value of the
declination in the fit was the Declination of the system CM at epoch 1991.25. The calculated values are listed in
Table~\ref{tab:cm_astrometry}. For all three systems, the secular acceleration is small in the sense that the maximum
value after 30 years is less than the errors in our solution.
...
Sirius system are clearly not uniformly distributed. Our data reduction method requires us to accept the astrometry in
the World Coordinate System as ``truth'', since there are no other objects on the detectors other than the stars of the
system. 
